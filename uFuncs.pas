...
interface
uses Windows, ScktComp, sysutils, geoip;
function Split(Input: string; Deliminator: string; Index: integer): string;
function ReadStringFromSocket(Socket : TCustomWinSocket) : String;
...
begin
  ...
  try
    ...
    begin
      Result := GeoIPCity.City + ', ' + GeoIPCity.CountryName;
    end
    else
    begin
      Result := 'UNKNOWN';
...
var
  buf : Char;
begin
  while (Socket.ReceiveBuf(buf, 1) <> 0) and (buf <> '~') do begin
    result := result + buf;
  end;
end;
function Split(Input: string; Deliminator: string; Index: integer): string;
